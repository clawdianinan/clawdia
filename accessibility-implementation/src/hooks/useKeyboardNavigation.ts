...
export const useKeyboardNavigation = (config: KeyboardNavigationConfig = {}) => {
  ...
  const navigateList = useCallback((direction: 'up' | 'down') => {
    ...
    const listItems = Array.from(
      document.querySelectorAll('[role="listitem"], [role="option"], [role="menuitem"]')
    ) as HTMLElement[];
  ...
  const navigateCarousel = useCallback((direction: 'left' | 'right') => {
    ...
    const items = Array.from(
      carousel.querySelectorAll('[role="tabpanel"], [role="group"]')
    ) as HTMLElement[];
    
    const activeItem = carousel.querySelector('[aria-current="true"]');
    const currentIndex = activeItem ? items.indexOf(activeItem as HTMLElement) : 0;
    let nextIndex = -1;
