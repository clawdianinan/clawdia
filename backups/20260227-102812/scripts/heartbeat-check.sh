...
check_urgent_escalations() {
    ...
    if [ -f "$OPENCLAW_WORKSPACE"/*.md ]; then
...
check_heartbeat_ok_conditions() {
    ...
    if check_urgent_escalations > /dev/null; then
        [ $? -gt 0 ] && violations=$((violations + 1))
    fi
    
    # Check for blocked tasks (simulated)
    # This would check task management system
    log "INFO" "Blocked tasks check skipped (requires task system integration)"
    
    # Check for deadline risks (simulated)
    # This would check calendar/task deadlines
    log "INFO" "Deadline risks check skipped (requires deadline tracking)"
    
    # Check for meeting prep gaps (simulated)
    log "INFO" "Meeting prep check skipped (requires calendar integration)"
    
    # Check for unresolved high-priority threads (simulated)
    ...
    if [ $violations -eq 0 ]; then
        ...
        log "WARN" "HEARTBEAT_OK conditions not met ($violations violations)"
        return 1
    fi
}
