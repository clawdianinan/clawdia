...
if [ -f "$WORKSPACE/scripts/consolidated-email-processor.sh" ]; then
    ...
    EMAIL_CONTEXT_MODE="${EMAIL_CONTEXT_MODE:-iih}" "$WORKSPACE/scripts/consolidated-email-processor.sh" --summary 2>&1 | grep -E "(PRIORITY|ALERT|IHS Towers|Policy guard|SUCCESS|WARN)" | head -20 >> "$DIGEST_FILE" || echo "• Error checking emails" >> "$DIGEST_FILE"
else
    echo "• Consolidated email processor not found" >> "$DIGEST_FILE"
fi
echo "" >> "$DIGEST_FILE"
