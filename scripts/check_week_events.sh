...
# Can be run on demand or added to regular checks
...
tell application "Calendar"
    ...
    if targetCalendar is null then
        display dialog "ERROR: Calendar named 'temi' not found"
        return
    end if
    ...
    if weekCount = 0 then
        display dialog "No events scheduled for the current week in temi calendar"
        return
    end if
